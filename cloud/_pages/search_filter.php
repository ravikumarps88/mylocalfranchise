...
//---------------------------------------------------------------------------------------------------
if ($action=="delete") {
dbQuery("UPDATE franchise_pricerange SET status='deleted' WHERE id='{$_REQUEST['id']}'");
$INFO_MSG = "Price range has been set to status deleted.";	
}
//---------------------------------------------------------------------------------------------------
$status	= ($_REQUEST['status'] == 'all' ? '1' : "status='".($_REQUEST['status'] == 'deleted' ? 'deleted' : ($_REQUEST['status'] == 'inactive' ? 'inactive' : 'active'))."'");
//echo $status;exit;
$sort	= 'pricerange';
if($_REQUEST['sort'] == 'pricerange')
$sort	= 'pricerange';

$query = "select * from franchise_pricerange WHERE $status ORDER BY $sort";
//echo $query;exit;
...
<script>
    $(document).ready(function () {
        ...
        $('#status').change(function () {
            $(location).attr('href', 'index.php?_page=search_filter&status=' + $(this).val());
        });
    });
</script>
<? ############################################################################################## ?>
...
<a href="index.php?_page=add_edit_search_filter">
...
<table width="100%" border="0" cellspacing="0" cellpadding="4"  class="table table-bordered table-striped datatable" id="table-2">
    <thead>
        ...
        <tr>
            ...
            <td width="23%" align="left">Price Range</td>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td width="23%" align="left">
                <a href="index.php?_page=add_edit_search_filter&action=edit&id=<?= $recordsList[$i]['id'] ?>">
                    <strong><?= no_magic_quotes($recordsList[$i]['pricerange_title']) ?></strong>
                </a>
            </td>
            ...
            <td width="10%" align="left">
                <a href="index.php?_page=add_edit_search_filter&action=edit&id=<?= $recordsList[$i]['id'] ?>" class="btn btn-default btn-sm btn-icon icon-left">
                    <i class="entypo-pencil"></i>
                    Edit                
                </a>

                <a href="index.php?_page=search_filter&action=delete&id=<?= $recordsList[$i]['id'] ?>"  onclick="return confirm('Are you sure?');" class="btn btn-danger btn-sm btn-icon icon-left">
