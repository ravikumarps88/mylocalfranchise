...
if ($action=="save") {
	...
	dbPerform("news_categories", $dbFields, $specialFields, $cond);
	$action = "edit";
...
if($action == 'edit')	{
	$query = "select * from news_categories WHERE id='{$_REQUEST['id']}'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="4" class="table">
	  <tr onmouseover="this.className = 'mouseOverRow';" onmouseout="this.className = '';">
		<td colspan="3" >
        	<a href="index.php?_page=news_categories" id="back" class="button" style="float:right;">Back</a>&nbsp;&nbsp;&nbsp;<button style="float:right;"><a id="submit" style="cursor:pointer;">Save</a></button>
        </td>
	  </tr> 	
	  ...
	  <tr onmouseover="this.className = 'mouseOverRow';" onmouseout="this.className = '';">
		<td colspan="3">
        	Category:<br />
        	<input type="text" name="category" value="<?=$recordsList[0]['category']?>" id="category" class="pref"  />
        </td>
      </tr>
