...
if ($action=="save") {
	...
	if($_FILES['logo']['size'] > 0)	{
		$filename	= '../upload/vendors/lifestyle/original/'.$_FILES['logo']['name'];
		copy($_FILES['logo']['tmp_name'],$filename);
	
		$filename	= '../upload/vendors/lifestyle/thumbnail/'.$_FILES['logo']['name'];
	...
	//adding
        if($_REQUEST['id'] == '') {
            $dbFields['lifestyle'] = $franchise_lifestyle;
        }
	...
	$dbFields['status'] 		= $status;
        ...
        if($_REQUEST['id'] == '' && $url_title != '') {
            $lifestyleUrlQuery = "select id from franchise_lifestyle WHERE url_title LIKE '%{$url_title}%'";
            $lifestyleUrlData = dbQuery($lifestyleUrlQuery);
            $pricerangeUrlQuery = "select id from franchise_pricerange WHERE url_title LIKE '%{$url_title}%'";
            $priceraneUrlData = dbQuery($pricerangeUrlQuery);
	...
	$specialFields = array();
        if($urlErr == '') {
            if($_REQUEST['id'] != '')	{		
                    ...
                    $cond	= "id=".$_REQUEST['id'];
                    $INFO_MSG = "Lifestyle data has been edited.";
            }	
            else	{
                    $dbFields['inserted_on'] 		= 'now()';
                    $specialFields = array('inserted_on');	
                    $INFO_MSG = "Lifestyle data has been added.";
            }

            $new_id = dbPerform("franchise_lifestyle", $dbFields, $specialFields, $cond);
...
if($action == 'edit')	{
	...
	$query = "select * from franchise_lifestyle WHERE id='{$_REQUEST['id']}'";
...
<form id="form_search_filter" action="" method="post" enctype="multipart/form-data" class="validate">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="4" class="table">
        ...
        <tr onmouseover="this.className = 'mouseOverRow';" onmouseout="this.className = '';">
            ...
            <td width="18%">Lifestyle Title: <a href="#" data-original-title="This is the title of lifestyle"  data-toggle="tooltip" data-placement="top"  class="tooltip-primary" border="0"><i class="entypo-info-circled"></i></a></td>
            <td><input type="text" name="lifestyle_title" value="<?= no_magic_quotes($recordsList[0]['lifestyle_title']) ?>" id="field-1" class="form-control"  /></td>
        </tr> 
        ...
        <tr onmouseover="this.className = 'mouseOverRow';" onmouseout="this.className = '';">
            ...
            <td>
                ...
                <img src="../upload/vendors/lifestyle/thumbnail/<?= $recordsList[0]['image'] ?>" class="img-responsive img-rounded" style="margin-top:5px;" />
            </td>
